            ...
            <div className="hidden md:flex items-center space-x-1">
              {navItems.map((item) =>
                !item.dropdown ? (
                  ...
                  >
                    <button
                      className={`px-3 py-2 rounded-md text-sm font-medium text-gray-700 dark:text-gray-200 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-all duration-200 flex items-center ${activeDropdown === item.name ? 'text-indigo-600 dark:text-indigo-400 bg-gray-50 dark:bg-gray-800' : ''
                        }`}
                    >
                      {item.icon}
                      {item.name}
                    ...
                    <div
                      className={`absolute left-0 mt-2 w-48 rounded-lg shadow-lg bg-white dark:bg-gray-900 ring-1 ring-black ring-opacity-5 dark:ring-gray-700 transition-all duration-200 ${activeDropdown === item.name ? 'opacity-100 visible transform translate-y-0' : 'opacity-0 invisible transform -translate-y-2'
                        }`}
                    >
                      <div className="py-1">
            ...
            {navItems.map((item) =>
              !item.dropdown ? (
                ...
                <div key={item.name}>
                  ...
                  <div
                    className={`pl-4 mt-1 space-y-1 transition-all duration-200 ${activeDropdown === item.name ? 'block' : 'hidden'
                      }`}
                  >
                    {item.items?.map((subItem) => (
                      <a
